...
void adjproject0(int k, int n, int count,func_force_t force )
{
  ...
  extern int qpts, dimR, Nx, Nz;
	...
	smMult0(Rps, MC[count][0][ALPHA], tm, RSDIAG-1, RSDIAG-1, dimR);
	for (i = 0; i < dimR; ++i)
	{
 ...
 /* MZ = M0 - (1/RE)b[k]dt*D0 */
    ...
    for (i = 0; i < dimR; ++i)
    {
      for (j = 0; j < qpts; ++j)
	{
	  Re(fa[i]) += Rw[i][j]*(Re(U[0][HXEL][j][0]))+Rpw[i][j]*Re(U[0][DZEL][j][0]);
	}
      if(force!=NULL)
	{
	  Re(fa[i])+=Re(tmp[i]);
    ...
    for (i = 0; i < dimR; ++i)
    {
      for (j = 0; j < qpts; ++j)
	{
	  Re(fb[i]) += Rw[i][j] * Re(U[0][HZEL][j][0])-Rpw[i][j]*Re(U[0][DXEL][j][0]);
	}
  ...
  flux_t=0;
	...
	for (i = 0; i < dimR; ++i)
	  {
	    for (j = 0; j < qpts; ++j)
	      {
		Re(fa[i]) += Rw[i][j]*(Re(LU[0][HXEL][j][0]))+Rpw[i][j]*Re(LU[0][DZEL][j][0]);
	      }
	    if(force !=NULL)
	      {
		Re(fa[i])+=Re(tmp[i]);
	...
	smMult0(Rps, MC[count-1][0][ALPHA], tm, RSDIAG-1, RSDIAG-1, dimR);
	for (i = 0; i < dimR; ++i)
	{
	  Re(AC[0][ALPHA][i][0]) -= Re(tm[i])*dt*re*(a[k+1]+b[k+1]);
	  Im(AC[0][ALPHA][i][0]) -= Im(tm[i])*dt*re*(a[k+1]+b[k+1]);
	}
	...
	for (i = 0; i < dimR; ++i)
	  {
	    for (j = 0; j < qpts; ++j)
	      {
		Re(fb[i]) += Rw[i][j] * Re(LU[0][HZEL][j][0])-Rpw[i][j]*Re(LU[0][DXEL][j][0]);
	      }
	     if(force !=NULL)
	      {
		Re(fb[i])+=Re(tmp2[i]);
	...
	smMult0(Rps, MC[count-1][0][BETA], tm, RSDIAG-1, RSDIAG-1, dimR);
        for (i = 0; i < dimR; ++i)
	{
	  Re(AC[0][BETA][i][0]) -= Re(tm[i])*dt*re*(a[k+1]+b[k+1]);
	  Im(AC[0][BETA][i][0]) -= Im(tm[i])*dt*re*(a[k+1]+b[k+1]);
	}
    }
