...
int main(int argc, char **argv)
{
  /*****************************Definition of all variables ****************************/
    ...
    if (argc != 11) 
    {
	printf("Required arguments are Nx,Ny,Nz,Lx,Lz,dt,tsteps,mpg,Re, restart_flag.\n");
	return(EXIT_FAILURE);
    }
  ...
  if(Ny-4<0)
	...
	freedVector(Uadd);freedMatrix(Rpw); freedMatrix(Qppw);freedVector(Qy);
	freedVector(W);freedVector(Rpp0);
	return(EXIT_FAILURE);
    }
    ...
    if ( getMem() != NO_ERR )
    {
        ...
        freedMatrix(Rps); freedVector(Rp0); freedVector(Vadd);freedVector(Vpadd);
	freedVector(Uadd); freedMatrix(Rpw); freedMatrix(Qppw);freedVector(Qy);
	freedVector(W); freedVector(Kx); freedVector(Kz); freedMatrix(K2);
	freedVector(Rpp0);
	return(EXIT_FAILURE);
    }
	...
	freecMatrix(IAUxb);	freecMatrix(IAUzb);freedVector(Rpp0);
        return(EXIT_FAILURE);
    }
    ...
    memcpy(MC[0][0][0][0],C[0][0][0], (Nz)*2*(Ny-2)*(Nx/2)*sizeof(mcomplex));
    memcpy(MIC[0][0][0][0],IC[0][0][0], (Nz)*2*(Ny-2)*(Nx/2)*sizeof(mcomplex));
    count=0;
