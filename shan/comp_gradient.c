...
int comp_gradient(int n, int k)
{
 /* External Variables */
    ...
    extern mcomplex  **AUxb, **AUzb, **grad;
  ...
  for (z=0; z<Nz/2; ++z)
	...
	for (z = 0; z < (3*Nz/2); ++z)
	{
	    for (x = 0; x < 3*Nx/2; ++x)
	    {
	      RT[idx*2+(z*(3*Nx/2+2)+x)] = RT[(z*(3*Nx/2+2)+x)]*RT[2*idx+(z*(3*Nx/2+2)+x)];
	      RT[idx*3+(z*(3*Nx/2+2)+x)] = RT[idx+(z*(3*Nx/2+2)+x)]*RT[3*idx+(z*(3*Nx/2+2)+x)];
	      RT[(z*(3*Nx/2+2)+x)] = RT[(z*(3*Nx/2+2)+x)]*RT[(z*(3*Nx/2+2)+x)];
	      RT[idx+(z*(3*Nx/2+2)+x)] = RT[idx+(z*(3*Nx/2+2)+x)]*RT[idx+(z*(3*Nx/2+2)+x)];
	    }
	}
	...
	for (z=0; z< Nz/2; z++)
	  {
	    for(x=0; x< Nx/2; x++)
	      {
		Re(grad[z][x])-=(re*(Re(CT[0][z][x])+Re(CT[1][z][x]))/2.+re*(Re(CT[2][z][x])+Re(CT[3][z][x])))*dt;
		Im(grad[z][x])-=(re*(Im(CT[0][z][x])+Im(CT[1][z][x]))/2.+re*(Im(CT[2][z][x])+Im(CT[3][z][x])))*dt;
	      }
	  }
	for (z=Nz/2+1; z< Nz; z++)
	  {
	    for(x=0; x< Nx/2; x++)
	      {
		Re(grad[z][x])-=(re*(Re(CT[0][z+Nz/2][x])+Re(CT[1][z+Nz/2][x]))/2.+re*(Re(CT[2][z+Nz/2][x])+Re(CT[3][z+Nz/2][x])))*dt;
		Im(grad[z][x])-=(re*(Im(CT[0][z+Nz/2][x])+Im(CT[1][z+Nz/2][x]))/2.+re*(Im(CT[2][z+Nz/2][x])+Im(CT[3][z+Nz/2][x])))*dt;
	      }
	  }
