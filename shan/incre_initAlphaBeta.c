...
void incre_Alpha0(void)
{
    ...
    for (i = 0; i < dimR; ++i)
    {
        ...
        for (j = 0; j < qpts; ++j)
	{
	  Re(IC[0][ALPHA][i][0]) += Rw[i][j]*(Re(IU[0][XEL][j][0])-Re(Uxb[0][0])*(1-Qy[j])/2.);
	  Im(IC[0][ALPHA][i][0]) += Rw[i][j]*(Im(IU[0][XEL][j][0])-Im(Uxb[0][0])*(1-Qy[j])/2.);
	}
    }
...
void incre_Beta0(void)
{
    ...
    for (i = 0; i < dimR; ++i)
    {
        ...
        for (j = 0; j < qpts; ++j)
	{
	  Re(IC[0][BETA][i][0]) += Rw[i][j]*(Re(IU[0][ZEL][j][0])-Re(Uzb[0][0])*(1-Qy[j])/2.);
	  Im(IC[0][BETA][i][0]) += Rw[i][j]*(Im(IU[0][ZEL][j][0])-Im(Uzb[0][0])*(1-Qy[j])/2.);
	}
    }
...
void incre_Alpha(int z, int x0, int xdim)
{
        ...
        for (j = 0; j < qpts; ++j)
	{
            for (x = x0; x < xdim; ++x)
	    {
	      Re(IC[z][ALPHA][i][x]) += Qw[i][j]*(Re(IU[z][YEL][j][x])-Re(Uzb[z][x])/4.*(1-Qy[j])*(1-Qy[j]*Qy[j]));
	      Im(IC[z][ALPHA][i][x]) += Qw[i][j]*(Im(IU[z][YEL][j][x])-Im(Uzb[z][x])/4.*(1-Qy[j])*(1-Qy[j]*Qy[j]));
	    }
	}
    }
...
void incre_Beta(int z, int x0, int xdim)
{
        ...
        for (j = 0; j < qpts; ++j)
	{
            for (x = x0; x < xdim; ++x)
	    {
		  ...
		  ((Kx[x]*Im(IU[z][ZEL][j][x]) - Kz[z]*Im(IU[z][XEL][j][x]))-Re(Uxb[z][x])/2.*(1-Qy[j]));
                Im(IC[z][BETA][i][x]) += Rw[i][j] * 
		  ((Kz[z]*Re(IU[z][XEL][j][x]) - Kx[x]*Re(IU[z][ZEL][j][x]))-Im(Uxb[z][x])/2.*(1-Qy[j]));
	    }
	}
    }
