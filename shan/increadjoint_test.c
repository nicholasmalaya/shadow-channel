...
int main(int argc, char **argv)
{
  ...
  if(Ny-4<0)
	...
	freedVector(Uadd);freedMatrix(Rpw);
	return(EXIT_FAILURE);
    }
		...
		for (z=0; z<Nz; ++z)
		  {
			...
			if(x==0 && z==0)
			  {
			err2=  (Re(AU[z][XEL][y][x])-(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-0.01)))* 
			  (Re(AU[z][XEL][y][x])-(1-Qy[y]*Qy[y])*sin((n*dt-dt-0.01)*2*M_PI))*W[y]
		...
		for (z=0; z<Nz; ++z)
		  {
			...
			if(x==0 && z==0)
			  {
			err2=  (Re(IAU[z][XEL][y][x])-(2-Qy[y]-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-0.01)))* 
			  (Re(IAU[z][XEL][y][x])-(2-Qy[y]-Qy[y]*Qy[y])*sin((n*dt-dt-0.01)*2*M_PI))*W[y]
			...
			err=err2+err;
			  ...
			  {
			err2=  (Re(IAU[z][XEL][y][x])-(1-Qy[y])*sin(2*M_PI*(n*dt-dt-0.01)))* 
			  (Re(IAU[z][XEL][y][x])-(1-Qy[y])*sin((n*dt-dt-0.01)*2*M_PI))*W[y]
			      +Im(IAU[z][XEL][y][x])*Im(IAU[z][XEL][y][x])*W[y]+
		        	  Re(IAU[z][YEL][y][x])*Re(IAU[z][YEL][y][x])*W[y]
			      +Im(AU[z][YEL][y][x])*Im(IAU[z][YEL][y][x])*W[y]+
			   Re(IAU[z][ZEL][y][x])*Re(IAU[z][ZEL][y][x])*W[y]
			  +Im(IAU[z][ZEL][y][x])*Im(IAU[z][ZEL][y][x])*W[y];
			err=err2+err;
			}
