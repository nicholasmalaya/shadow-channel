...
int  write_data2(int n)
{
    ...
    hid_t fid2, dataset_Nx, dataset_Ny, dataset_Nz, dataset_dt, dataset_Re, dataset_mpg;
    hid_t complex_id;
	...
	for (z=0; z<Nz; ++z)
	  {
		...
		IMatrix2[y][z][x].im=Im(IC[z][BETA][y][x]);
	      ...
	      dataset_a = H5Dcreate(file_id1, "/data_alpha", complex_id, fid1, H5P_DEFAULT);
	      dataset_b = H5Dcreate(file_id1, "/data_beta", complex_id, fid1, H5P_DEFAULT);

	      dataset_ia = H5Dcreate(file_id1, "/data_ialpha", complex_id, fid1, H5P_DEFAULT);
	      dataset_ib = H5Dcreate(file_id1, "/data_ibeta", complex_id, fid1, H5P_DEFAULT);


	      dataset_Nx=H5Dcreate(file_id1, "data_Nx", H5T_STD_I32LE, fid2,  H5P_DEFAULT);
	      dataset_Ny=H5Dcreate(file_id1, "data_Ny", H5T_STD_I32LE, fid2,  H5P_DEFAULT);
	      dataset_Nz=H5Dcreate(file_id1, "data_Nz", H5T_STD_I32LE, fid2,  H5P_DEFAULT);

	      dataset_dt=H5Dcreate(file_id1, "data_dt", H5T_IEEE_F64LE, fid2,  H5P_DEFAULT);
	      dataset_Re=H5Dcreate(file_id1, "data_Re", H5T_IEEE_F64LE, fid2,  H5P_DEFAULT);
	      dataset_mpg=H5Dcreate(file_id1, "data_mpg", H5T_IEEE_F64LE, fid2,  H5P_DEFAULT);

	      /* write data to corresponding datasets*/
	      ret = H5Dwrite(dataset_a,complex_id, H5S_ALL,fid1 , H5P_DEFAULT, Matrix1);
	      ret = H5Dwrite(dataset_b,complex_id, H5S_ALL,fid1 , H5P_DEFAULT, Matrix2);
	      ret = H5Dwrite(dataset_ia,complex_id, H5S_ALL,fid1 , H5P_DEFAULT, IMatrix1);
	      ret = H5Dwrite(dataset_ib,complex_id, H5S_ALL,fid1 , H5P_DEFAULT, IMatrix2);

	      ret = H5Dwrite(dataset_Nx,H5T_NATIVE_INT, H5S_ALL,fid2 , H5P_DEFAULT, &Nx);
	      ret = H5Dwrite(dataset_Ny,H5T_NATIVE_INT, H5S_ALL,fid2 , H5P_DEFAULT, &Ny);
	      ret = H5Dwrite(dataset_Nz,H5T_NATIVE_INT, H5S_ALL,fid2 , H5P_DEFAULT, &Nz);
	      ret = H5Dwrite(dataset_dt,H5T_IEEE_F64LE , H5S_ALL,fid2 , H5P_DEFAULT, &dt);
	      ret = H5Dwrite(dataset_Re,H5T_IEEE_F64LE , H5S_ALL,fid2 , H5P_DEFAULT, &re);
	      ret = H5Dwrite(dataset_mpg,H5T_IEEE_F64LE , H5S_ALL,fid2 , H5P_DEFAULT, &mpg);
