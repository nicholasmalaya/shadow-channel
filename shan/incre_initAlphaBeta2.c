...
void incre_initAlphaBeta_2(int z, int x0)
{
  ...
  for (x=x0; x< Nx/2; ++x)
	...
	for (j=0; j < dimR; ++j)
	  {
	    Re(IUxb[z][x]) += (Rp0[j]*Im(IC[z][BETA][j][x]))*Kz[z]/K2[z][x];
	    Im(IUxb[z][x]) +=(-Rp0[j]*Re(IC[z][BETA][j][x]))*Kz[z]/K2[z][x];

	    Re(IUzb[z][x]) +=-(Rp0[j]*Im(IC[z][BETA][j][x]))*Kx[x]/K2[z][x];
	    Im(IUzb[z][x]) += (Rp0[j]*Re(IC[z][BETA][j][x]))*Kx[x]/K2[z][x];
  ...
  for (x=x0; x< Nx/2; ++x)
	...
	for (j=0; j < dimQ; ++j)
	  {
	    sign=sign*(-1);
	    Re(IUxb[z][x]) -= (sign*8.*Im(IC[z][ALPHA][j][x]))*Kx[x]/K2[z][x];
	    Im(IUxb[z][x]) -=(-sign*8.*Re(IC[z][ALPHA][j][x]))*2*Kx[x]/K2[z][x];

	    Re(IUzb[z][x]) -= (sign*8.*Im(IC[z][ALPHA][j][x]))*Kz[z]/K2[z][x];
	    Im(IUzb[z][x]) -=(-sign*8.*Re(IC[z][ALPHA][j][x]))*2*Kz[z]/K2[z][x];
	  }
