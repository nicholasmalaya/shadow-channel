...
void adjproject(int n, int k, int z, int x0, int count,func_force_t force )
{
    ...
    extern double **Q, **Qp, **Qpp, **R, **Rp, **Qw, **Qpw, **Rw, **Qs, **Qps,
      **Qpps, **Rs, **Rps, *Rp0, **Qppw, **Rpw, *Qy, *Rpp0;
    extern double ***M;
  ...
  if(force!=NULL)
    ...
    if (k == 0)		/* first step */
    {
	   ...
	   in TM.  Then transfer the result back to C[z][ALPHA]. */
        smMult(M, AC[z][ALPHA], TM, QSDIAG-1, QSDIAG-1, dimQ, Nx/2, x0);
	for (i = 0; i < dimQ; ++i)
	{
	    memcpy(&AC[z][ALPHA][i][x0], &TM[i][x0], (Nx/2-x0)*sizeof(mcomplex));
	}
	...
	smMult(M, MC[count][z][ALPHA], TM, QSDIAG-1, QSDIAG-1, dimQ, Nx/2, x0);
	for (i = 0; i < dimQ; ++i)
	{
	  for (x=x0; x< Nx/2; x++)
	    {
	      Re(AC[z][ALPHA][i][x]) += Re(TM[i][x])*re*dt*(a[0]+b[0]);
	      Im(AC[z][ALPHA][i][x]) += Im(TM[i][x])*re*dt*(a[0]+b[0]); 
	    }
	}
        ...
        for (j = 0; j < qpts; ++j)
	{
	    for (x = x0; x < Nx/2; ++x)
	    {
	        Re(Fa[i][x]) += ( Qpw[i][j] *
	          ...
	          K2[z][x]*(Qw[i][j]*Re(U[z][HYEL][j][x])) )+ Qppw[i][j]*
		  (Kz[z]*Im(U[z][DXEL][j][x])-Kx[x]*Im(U[z][DZEL][j][x]));

	        Im(Fa[i][x]) += ( Qpw[i][j] *
		  (Kx[x]*Re(U[z][HXEL][j][x]) + Kz[z]*Re(U[z][HZEL][j][x])) -
	           K2[z][x]*(Qw[i][j]*Im(U[z][HYEL][j][x])) )+ Qppw[i][j]*
		  (-Kz[z]*Re(U[z][DXEL][j][x])+Kx[x]*Re(U[z][DZEL][j][x]));
	    }
	}
    }
    ...
    if (k == 0)		/* first step */
    {
	   ...
	   in TM.  Then transfer the result back to C[z][ALPHA]. */
        smMult(M, AC[z][BETA], TM, RSDIAG-1, RSDIAG-1, dimR, Nx/2, x0);
	for (i = 0; i < dimR; ++i)
	{
	    memcpy(&AC[z][BETA][i][x0], &TM[i][x0], (Nx/2-x0)*sizeof(mcomplex));
	}
	...
	smMult(M, MC[count][z][BETA], TM, RSDIAG-1, RSDIAG-1, dimR, Nx/2, x0);
	for (i = 0; i < dimR; ++i)
	{
	  for (x=x0; x< Nx/2; x++)
	    {
	      Re(AC[z][BETA][i][x]) += Re(TM[i][x])*re*dt*(a[0]+b[0]);
	      Im(AC[z][BETA][i][x]) += Im(TM[i][x])*re*dt*(a[0]+b[0]); 
	    }
	}
    }
        ...
        for (j = 0; j < qpts; ++j)
	{
	    for (x = x0; x < Nx/2; ++x)
	    {
		  ...
		  Rpw[i][j]*(-Kz[z]*Im(U[z][DZEL][j][x])-Kx[x]*Im(U[z][DXEL][j][x]));
	        Im(Fb[i][x]) += Rw[i][j] *
		  (Kz[z]*Re(U[z][HXEL][j][x]) - Kx[x]*Re(U[z][HZEL][j][x]))+
		  Rpw[i][j]*(Kz[z]*Re(U[z][DZEL][j][x])+Kx[x]*Re(U[z][DXEL][j][x]));
	    }
	}
    }
    if(force!=NULL)
        ...
        for (j = 0; j < dimQ; ++j)
	{
            for (x = x0; x < Nx/2; ++x)
	    {
	        Re(AU[z][DXEL][i][x]) += Qpp[i][j]*Re(AC[z][ALPHA][j][x]);
	        Im(AU[z][DXEL][i][x]) += Qpp[i][j]*Im(AC[z][ALPHA][j][x]);
	    }
	}
    }
	...
	for (x = x0; x < Nx/2; ++x)
	{
	   ...
	   Re(AU[z][XEL][i][x]) = (Kx[x]*Im(AU[z][XEL][i][x]) +
	      Kz[z]*Im(AU[z][ZEL][i][x])) / K2[z][x];
	   Re(AU[z][ZEL][i][x]) = (-Kx[x]*Im(AU[z][ZEL][i][x]) +
	      Kz[z]*Im(AU[z][XEL][i][x])) / K2[z][x];
	   Im(AU[z][XEL][i][x]) = -(Kx[x]*t[0] + Kz[z]*t[1]) / K2[z][x];
	   Im(AU[z][ZEL][i][x]) = (Kx[x]*t[1] - Kz[z]*t[0]) / K2[z][x];
	}
    }
	...
	for (x = x0; x < Nx/2; ++x)
	{
	   ...
	   Re(AU[z][DXEL][i][x]) = (-Kx[x]*Im(AU[z][DXEL][i][x]) +
	      Kz[z]*Im(AU[z][DZEL][i][x])) / K2[z][x];
	   Re(AU[z][DZEL][i][x]) = -(Kx[x]*Im(AU[z][DZEL][i][x]) +
	      Kz[z]*Im(AU[z][DXEL][i][x])) / K2[z][x];
	   Im(AU[z][DXEL][i][x]) = (Kx[x]*t[0] - Kz[z]*t[1]) / K2[z][x];
	   Im(AU[z][DZEL][i][x]) = (Kx[x]*t[1] + Kz[z]*t[0]) / K2[z][x];
	}
    }
	...
	for (j=0; j < dimR; ++j)
	  {
	    Re(Uxb[z][x]) += Rp0[j]*Im(AC[z][BETA][j][x])*Kz[z]/K2[z][x];
	    Im(Uxb[z][x]) +=-Rp0[j]*Re(AC[z][BETA][j][x])*Kz[z]/K2[z][x];

	    Re(Uzb[z][x]) +=-Rp0[j]*Im(AC[z][BETA][j][x])*Kx[x]/K2[z][x];
	    Im(Uzb[z][x]) += Rp0[j]*Re(AC[z][BETA][j][x])*Kx[x]/K2[z][x];
	  }
      }
   for (x=x0; x< Nx/2; ++x)
      {
	for (j=0; j < dimR; ++j)
	  {
	    Re(HAUxb[z][x]) += Rpp0[j]*Im(AC[z][BETA][j][x])*Kz[z]/K2[z][x];
	    Im(HAUxb[z][x]) +=-Rpp0[j]*Re(AC[z][BETA][j][x])*Kz[z]/K2[z][x];

	    Re(HAUzb[z][x]) +=-Rpp0[j]*Im(AC[z][BETA][j][x])*Kx[x]/K2[z][x];
	    Im(HAUzb[z][x]) += Rpp0[j]*Re(AC[z][BETA][j][x])*Kx[x]/K2[z][x];
	  }
      }
	...
	for (i = 0; i < dimQ; ++i)
	  {
		...
		for (x = x0; x < Nx/2; ++x)
		  {
		    Re(Fa[i][x]) += ( Qpw[i][j] *
				      (-Kx[x]*Im(LU[z][HXEL][j][x]) - Kz[z]*Im(LU[z][HZEL][j][x])) -
				      K2[z][x]*(Qw[i][j]*Re(LU[z][HYEL][j][x])) )+ Qppw[i][j]*
		      (Kz[z]*Im(LU[z][DXEL][j][x])-Kx[x]*Im(LU[z][DZEL][j][x]));

		    Im(Fa[i][x]) += ( Qpw[i][j] *
				      (Kx[x]*Re(LU[z][HXEL][j][x]) + Kz[z]*Re(LU[z][HZEL][j][x])) -
				      K2[z][x]*(Qw[i][j]*Im(LU[z][HYEL][j][x])) )+ Qppw[i][j]*
		      (-Kz[z]*Re(LU[z][DXEL][j][x])+Kx[x]*Re(LU[z][DZEL][j][x]));
		  }
	      }
	  }
	if(force!=NULL)
	...
	for (i = 0; i < dimQ; ++i)
	{
	    for (x = x0; x < Nx/2; ++x)
	    {
		Re(AC[z][ALPHA][i][x]) = Re(TM[i][x]) + dt*d[k+1]*Re(Fa[i][x]);
		Im(AC[z][ALPHA][i][x]) = Im(TM[i][x]) + dt*d[k+1]*Im(Fa[i][x]);
	    }
	}
	memset(M[0][0], 0, dimR*9*(Nx/2)*sizeof(double));
	for (i = 0; i < dimQ; ++i) 	
	...
	smMult(M, MC[count-1][z][ALPHA], TM, QSDIAG-1, QSDIAG-1, dimQ, Nx/2, x0);

	for (i = 0; i < dimQ; ++i)
	{
	  for (x=x0; x< Nx/2; x++)
	    {
	      Re(AC[z][ALPHA][i][x]) += Re(TM[i][x])*re*dt*(a[k+1]+b[k+1]);
	      Im(AC[z][ALPHA][i][x]) += Im(TM[i][x])*re*dt*(a[k+1]+b[k+1]); 
	    }
	}
	...
	for (i = 0; i < dimR; ++i)
	  {
		...
		for (x = x0; x < Nx/2; ++x)
		  {
		    Re(Fb[i][x]) += Rw[i][j] *
		      (-Kz[z]*Im(LU[z][HXEL][j][x]) + Kx[x]*Im(LU[z][HZEL][j][x]))+
		      Rpw[i][j]*(-Kz[z]*Im(LU[z][DZEL][j][x])-Kx[x]*Im(LU[z][DXEL][j][x]));
		    Im(Fb[i][x]) += Rw[i][j] *
		      (Kz[z]*Re(LU[z][HXEL][j][x]) - Kx[x]*Re(LU[z][HZEL][j][x]))+
		      Rpw[i][j]*(Kz[z]*Re(LU[z][DZEL][j][x])+Kx[x]*Re(LU[z][DXEL][j][x]));
		  }
	      }
	  }
	...
	for (i = 0; i < dimR; ++i)
	  {
	    for (x = x0; x < Nx/2; ++x)
	      {
		Re(AC[z][BETA][i][x]) = Re(TM[i][x]) + dt*d[k+1]*Re(Fb[i][x]);
		Im(AC[z][BETA][i][x]) = Im(TM[i][x]) + dt*d[k+1]*Im(Fb[i][x]);
	      }
	  }
	...
	smMult(M, MC[count-1][z][BETA], TM, RSDIAG-1, RSDIAG-1, dimR, Nx/2, x0);
	for (i = 0; i < dimR; ++i)
	{
	  for (x=x0; x< Nx/2; x++)
	    {
	      Re(AC[z][BETA][i][x]) += Re(TM[i][x])*re*dt*(a[k+1]+b[k+1]);
	      Im(AC[z][BETA][i][x]) += Im(TM[i][x])*re*dt*(a[k+1]+b[k+1]); 
	    }
	}
