...
int comp_hess(int n, int k)
{
 /* External Variables */
    ...
    extern mcomplex  **AUxb, **AUzb, **hess;
    ...
    static  double e[3] = { 1., -1./3, 1./3.};
    memset(CT[0][0], 0, 9*(3*Nz/2)*(3*Nx/4+1)*sizeof(fftw_complex));
    memset(ICT[0][0], 0, 9*(3*Nz/2)*(3*Nx/4+1)*sizeof(fftw_complex));
  ...
  for (z=0; z<Nz/2; ++z)
	...
	memcpy(CT[2][z+Nz/2], GIUxb[z], (Nx/2)*sizeof(fftw_complex));
	memcpy(CT[3][z+Nz/2], GIUzb[z], (Nx/2)*sizeof(fftw_complex));
	memcpy(CT[4][z+Nz/2], IUxb[z], (Nx/2)*sizeof(fftw_complex));
	memcpy(CT[5][z+Nz/2], IUzb[z], (Nx/2)*sizeof(fftw_complex));

	memcpy(CT[6][z+Nz/2], IAUxb[z], (Nx/2)*sizeof(fftw_complex));
	memcpy(CT[7][z+Nz/2], IAUzb[z], (Nx/2)*sizeof(fftw_complex));

	memcpy(ICT[0][z+Nz/2], HUxb[z], (Nx/2)*sizeof(fftw_complex));
	memcpy(ICT[1][z+Nz/2], HUzb[z], (Nx/2)*sizeof(fftw_complex));

	memcpy(ICT[2][z+Nz/2], HAUxb[z], (Nx/2)*sizeof(fftw_complex));
	memcpy(ICT[3][z+Nz/2], HAUzb[z], (Nx/2)*sizeof(fftw_complex));
      }
	...
	for (z = 0; z < (3*Nz/2); ++z)
	{
	    for (x = 0; x < 3*Nx/2; ++x)
	    {
	      IRT[idx*2+(z*(3*Nx/2+2)+x)]=(RT[(z*(3*Nx/2+2)+x)]*IRT[2*idx+(z*(3*Nx/2+2)+x)]+
					   IRT[(z*(3*Nx/2+2)+x)]*RT[2*idx+(z*(3*Nx/2+2)+x)])*IRT[4*idx+(z*(3*Nx/2+2)+x)];
	      tmp1=IRT[idx+(z*(3*Nx/2+2)+x)];
	      IRT[idx+(z*(3*Nx/2+2)+x)]=(RT[idx+(z*(3*Nx/2+2)+x)]*IRT[3*idx+(z*(3*Nx/2+2)+x)]+
					 IRT[idx+(z*(3*Nx/2+2)+x)]*RT[3*idx+(z*(3*Nx/2+2)+x)])*IRT[4*idx+(z*(3*Nx/2+2)+x)];
	      IRT[(z*(3*Nx/2+2)+x)]=(RT[(z*(3*Nx/2+2)+x)]*IRT[(z*(3*Nx/2+2)+x)]+
				     tmp1*RT[idx+(z*(3*Nx/2+2)+x)])*IRT[4*idx+(z*(3*Nx/2+2)+x)];
	    }
	}
	...
	for (z = 0; z < (3*Nz/2); ++z)
	{
	    for (x = 0; x < 3*Nx/2; ++x)
	    {
	      RT[2*idx+z*(3*Nx/2+2)+x]= RT[2*idx+(z*(3*Nx/2+2)+x)]*RT[4*idx+(z*(3*Nx/2+2)+x)]+
		RT[3*idx+(z*(3*Nx/2+2)+x)]*RT[5*idx+(z*(3*Nx/2+2)+x)];
	      tmp1=RT[idx+(z*(3*Nx/2+2)+x)];
	      RT[idx+(z*(3*Nx/2+2)+x)]=	 RT[(z*(3*Nx/2+2)+x)]*RT[6*idx+(z*(3*Nx/2+2)+x)]+
		 RT[idx+(z*(3*Nx/2+2)+x)]*RT[7*idx+(z*(3*Nx/2+2)+x)];
	      RT[z*(3*Nx/2+2)+x]= RT[(z*(3*Nx/2+2)+x)]*RT[4*idx+(z*(3*Nx/2+2)+x)]+
	...
	for (z=0; z< Nz/2; z++)
	  {
	    for(x=0; x< Nx/2; x++)
	      {
		Re(hess[z][x])+=re*(-Re(CT[0][z][x])-Re(CT[1][z][x])-Re(CT[2][z][x])-Re(ICT[0][z][x])-
				    Re(ICT[1][z][x])-Re(ICT[2][z][x]))*dt;
		Im(hess[z][x])+=2*re*(-Im(CT[0][z][x])-Im(CT[1][z][x])-Im(CT[2][z][x])-Im(ICT[0][z][x])-
	...
	for (z=Nz/2+1; z< Nz; z++)
	  {
	    for(x=0; x< Nx/2; x++)
	      {
		Re(hess[z][x])+=re*(-Re(CT[0][z+Nz/2][x])-Re(CT[1][z+Nz/2][x])-Re(CT[2][z+Nz/2][x])-Re(ICT[0][z+Nz/2][x])-
				    Re(ICT[1][z+Nz/2][x])-Re(ICT[2][z+Nz/2][x]))*dt;
		Im(hess[z][x])+=2*re*(-Im(CT[0][z+Nz/2][x])-Im(CT[1][z+Nz/2][x])-Im(CT[2][z+Nz/2][x])-Im(ICT[0][z+Nz/2][x])-
				    Im(ICT[1][z+Nz/2][x])-Im(ICT[2][z+Nz/2][x]))*dt;
	  }
	  }
	return(NO_ERR);
}
