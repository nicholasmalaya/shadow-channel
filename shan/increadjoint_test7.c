  ...
  memcpy(MIC[count][0][0][0],IC[0][0][0], (Nz)*2*(Ny-2)*(Nx/2)*sizeof(mcomplex));
   ...
   /* time step for forward problem */
	...
	for (dctr = 0; dctr < 3; ++dctr)     /* RK steps */
	{
	  ...
	  project0(count, dctr, n, force0_7);           /* (kx, kz)=(0, 0), solve for a, b */
	  project(count, n, dctr, 0, 1, NULL);      /* (kx, kz)!=(0, 0), solve for alpha, beta */
	    ...
	    increproject(count, dctr, 0, 1, n, increforce_7);
	    for (z = 1; z < Nz; ++z)
	      {
		if (z == Nz/2) 
		{
		...
		increproject(count, dctr, z, 0,n, increforce_7); 
	      }
		...
		for (z=0; z<Nz; ++z)
		  {
			...
			if(x==0 && z==0)
			  {
			err2=  (Re(U[z][XEL][y][x])-(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt)))* 
			  (Re(U[z][XEL][y][x])-(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt)))*W[y]
			      +Im(U[z][XEL][y][x])*Im(U[z][XEL][y][x])*W[y]+
		        	  Re(U[z][YEL][y][x])*Re(U[z][YEL][y][x])*W[y]
			      +Im(U[z][YEL][y][x])*Im(U[z][YEL][y][x])*W[y]+
			  (Re(U[z][ZEL][y][x])+(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt)))*
			  (Re(U[z][ZEL][y][x])+(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt)))*W[y]
		...
		for (z=0; z<Nz; ++z)
		  {
			...
			if(x==1 && z==1)
			  {
			    err2=  (Re(IU[z][XEL][y][x])-(1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin(2*M_PI*(n*dt+dt)))* 
			  (Re(IU[z][XEL][y][x])-(1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin((n*dt+dt)*2*M_PI))*W[y]
			      +Im(IU[z][XEL][y][x])*Im(IU[z][XEL][y][x])*W[y]+
		        	  Re(IU[z][YEL][y][x])*Re(IU[z][YEL][y][x])*W[y]
			      +(Im(IU[z][YEL][y][x])-Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt)))* 
			      (Im(IU[z][YEL][y][x])-Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt))) *W[y]
			      +( Re(IU[z][ZEL][y][x])+(1-Qy[y])*sin(2*M_PI*(n*dt+dt)))* 
			      ( Re(IU[z][ZEL][y][x])+(1-Qy[y])*sin(2*M_PI*(n*dt+dt)))* W[y]
			...
			/*if (x==1 && z==1)
			  {
			 printf("%d, %d, %d,  %f+i%f, %f+i%f, %f+i%f, %f, %f, %f\n",x,z,y, Re(IU[z][XEL][y][x]), Im(IU[z][XEL][y][x]),
				   Re(IU[z][YEL][y][x]), Im(IU[z][YEL][y][x]),Re(IU[z][ZEL][y][x]), Im(IU[z][ZEL][y][x]),
			(1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin(2*M_PI*(n*dt+dt)),	Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt+dt)), -(1-Qy[y])*sin(2*M_PI*(n*dt+dt)));
			}*/
		  }
		  }
	      }
  ...
  for (n =restart_flag+MAXSTEP; n >restart_flag; --n)	     /* loop for each timestep */
	...
	for (dctr = 0; dctr < 3; ++dctr)     /* RK steps */
	  ...
	  if(dctr==2){
	    z=0;
	    x=0;
		   printf("HUxb[%d][%d]=%f+i%f, %f+i%f, %f, %f\n", z, x, Re(HUxb[z][x]),
			  Im(HUxb[z][x]),Re(HUzb[z][x]),Im(HUzb[z][x]),
			  -2*sin(2*M_PI*(n*dt-dt)),-2*sin(2*M_PI*(n*dt-dt))); 
			  }*/
	   ...
	   adjproject0(dctr, n, count, adjforce0_7);
	   ...
	   memcpy(GIUzb[0], Uzb[0], (Nz)*(Nx/2)*sizeof(fftw_complex));
	    ...
	    increadjproject(n, dctr, 0, 1, count, increadjforce_7);
	    for (z = 1; z < Nz; ++z)
	      {
		if (z == Nz/2) 
		{
		...
		increadjproject(n, dctr, z, 0, count, increadjforce_7); 
		}

	    /* if(dctr==2){
	    z=0;
	    x=0;
		   printf("HAUxb[%d][%d]=%f+i%f, %f+i%f, %f, %f\n", z, x, Re(HAUxb[z][x]),
			  Im(HAUxb[z][x]),Re(HAUzb[z][x]),Im(HAUzb[z][x]),
			  -2*sin(2*M_PI*(n*dt-dt-FT)), 2*sin(2*M_PI*(n*dt-dt-FT))); 
			  }*/
	    /* if(dctr==2){
	    z=1;
	    x=1;
		   printf("HAUxb[%d][%d]=%f+i%f, %f+i%f, %f, %f\n", z, x, Re(IAUxb[z][x]),
			  Im(IAUxb[z][x]),Re(IAUzb[z][x]),Im(IAUzb[z][x]),
			  -9*sin(2*M_PI*(n*dt-dt-FT)), sin(2*M_PI*(n*dt-dt-FT))); 
			  }*/
	    if(dctr==2)
	      {
		...
		for (z=0; z<Nz; ++z)
		  {
			...
			if(x==0 && z==0)
			  {
			err2=  (Re(AU[z][XEL][y][x])-(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			  (Re(AU[z][XEL][y][x])-(1-Qy[y]*Qy[y])*sin((n*dt-dt-FT)*2*M_PI))*W[y]
			      +Im(AU[z][XEL][y][x])*Im(AU[z][XEL][y][x])*W[y]+
		        	  Re(AU[z][YEL][y][x])*Re(AU[z][YEL][y][x])*W[y]
			      +Im(AU[z][YEL][y][x])*Im(AU[z][YEL][y][x])*W[y]+
			  (Re(AU[z][ZEL][y][x])+(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))*
			  (Re(AU[z][ZEL][y][x])+(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))*W[y]
		...
		for (z=0; z<Nz; ++z)
		  {
			...
			if(x==1 && z==1)
			  {
			    err2=  (Re(IAU[z][XEL][y][x])-(1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			  (Re(IAU[z][XEL][y][x])-(1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin((n*dt-dt-FT)*2*M_PI))*W[y]
			      +Im(IAU[z][XEL][y][x])*Im(IAU[z][XEL][y][x])*W[y]+
		        	  Re(IAU[z][YEL][y][x])*Re(IAU[z][YEL][y][x])*W[y]
			      +(Im(IAU[z][YEL][y][x])-Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			      (Im(IAU[z][YEL][y][x])-Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT))) *W[y]
			      +( Re(IAU[z][ZEL][y][x])+(1-Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			      ( Re(IAU[z][ZEL][y][x])+(1-Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* W[y]
			      +Im(IAU[z][ZEL][y][x])*Im(IAU[z][ZEL][y][x])*W[y];
			err=err2+err;
			norm+=((1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			  (-(1-Qy[y]+(1-Qy[y]*Qy[y])*4*Qy[y])*sin((n*dt-dt-FT)*2*M_PI))*W[y]
			      +Im(IAU[z][XEL][y][x])*Im(IAU[z][XEL][y][x])*W[y]+
		        	  Re(IAU[z][YEL][y][x])*Re(IAU[z][YEL][y][x])*W[y]
			      +(-Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			      (-Kx[x]*(1-Qy[y]*Qy[y])*(1-Qy[y]*Qy[y])*sin(2*M_PI*(n*dt-dt-FT))) *W[y]
			      +((1-Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* 
			      ( (1-Qy[y])*sin(2*M_PI*(n*dt-dt-FT)))* W[y]
			      +Im(IAU[z][ZEL][y][x])*Im(IAU[z][ZEL][y][x])*W[y];
			}
			else
			  {
