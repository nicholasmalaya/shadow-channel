...
int pass1(int dctr, int n)
{
      ...
      for(i=0; i< MAXT; i++)
	{
	  for(j=0; j< 3*Nz/2; j++)
	    {
	      memset(CT[i][j], 0, (3*Nx/4+1)*sizeof(fftw_complex));
	    }
	}
	...
	if (dctr == 0)
        {
            ...
            for (i = 0; i < 3*Nz/2; ++i)
            {
                for (j = 0; j < 3*Nx/2; ++j)
                {
                    t = fabs(RT[XEL*idx+(i*(3*Nx/2+2)+j)])*cfl1 + 
			fabs(RT[YEL*idx+(i*(3*Nx/2+2)+j)])*cfl2[y] +
                        fabs(RT[ZEL*idx+(i*(3*Nx/2+2)+j)])*cfl3;
	            if (t > CFL)
	                {   CFL = t;  }
                }
            }
	    ...
	    if (CFL >= CFL_MAX)
	    {
	      printf("Pass1:  CFL[%d]:%e.  CFL condition violated at step %d.\n",
		     y, CFL, n);
	        return(ERROR);
	    }
	    else{
	...
	/* Clear the array ICT */
	memset(ICT[0][0], 0, MAXT*(3*Nz/2)*(3*Nx/4+1)*sizeof(fftw_complex));
	...
	for (z = 0; z < (3*Nz/2); ++z)
	{
		...
		IRT[HXEL*idx+(z*(3*Nx/2+2)+x)] =
	           IRT[YEL*idx+(z*(3*Nx/2+2)+x)]*RT[WZEL*idx+(z*(3*Nx/2+2)+x)] -
		   IRT[ZEL*idx+(z*(3*Nx/2+2)+x)]*RT[WYEL*idx+(z*(3*Nx/2+2)+x)]+
                   RT[YEL*idx+(z*(3*Nx/2+2)+x)]*IRT[WZEL*idx+(z*(3*Nx/2+2)+x)] -
		  RT[ZEL*idx+(z*(3*Nx/2+2)+x)]*IRT[WYEL*idx+(z*(3*Nx/2+2)+x)];

		IRT[HYEL*idx+(z*(3*Nx/2+2)+x)] =
		   IRT[ZEL*idx+(z*(3*Nx/2+2)+x)]*RT[WXEL*idx+(z*(3*Nx/2+2)+x)] -
		   IRT[XEL*idx+(z*(3*Nx/2+2)+x)]*RT[WZEL*idx+(z*(3*Nx/2+2)+x)]+
                   RT[ZEL*idx+(z*(3*Nx/2+2)+x)]*IRT[WXEL*idx+(z*(3*Nx/2+2)+x)] -
		   RT[XEL*idx+(z*(3*Nx/2+2)+x)]*IRT[WZEL*idx+(z*(3*Nx/2+2)+x)];

		IRT[HZEL*idx+(z*(3*Nx/2+2)+x)] =
		   IRT[XEL*idx+(z*(3*Nx/2+2)+x)]*RT[WYEL*idx+(z*(3*Nx/2+2)+x)] -
		   Iuy*RT[WXEL*idx+(z*(3*Nx/2+2)+x)]+
                   RT[XEL*idx+(z*(3*Nx/2+2)+x)]*IRT[WYEL*idx+(z*(3*Nx/2+2)+x)] -
		  uy*IRT[WXEL*idx+(z*(3*Nx/2+2)+x)];
	

                /* next compute H=u x omega */
	    	RT[HXEL*idx+(z*(3*Nx/2+2)+x)] =
	           RT[YEL*idx+(z*(3*Nx/2+2)+x)]*RT[WZEL*idx+(z*(3*Nx/2+2)+x)] -
		   RT[ZEL*idx+(z*(3*Nx/2+2)+x)]*RT[WYEL*idx+(z*(3*Nx/2+2)+x)];
		RT[HYEL*idx+(z*(3*Nx/2+2)+x)] =
		   RT[ZEL*idx+(z*(3*Nx/2+2)+x)]*RT[WXEL*idx+(z*(3*Nx/2+2)+x)] -
		   RT[XEL*idx+(z*(3*Nx/2+2)+x)]*RT[WZEL*idx+(z*(3*Nx/2+2)+x)];
		RT[HZEL*idx+(z*(3*Nx/2+2)+x)] =
		   RT[XEL*idx+(z*(3*Nx/2+2)+x)]*RT[WYEL*idx+(z*(3*Nx/2+2)+x)] -
    ...
    if( dctr==0 )
      {
	printf("The maximum time step size at time %d can be %f\n", n, maxtstep);
	maxtstep=10.;
