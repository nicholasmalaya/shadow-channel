...
try: import matplotlib; matplot.use('agg')
except: pass
...
class IsoBox(object):
    ...
    def __init__(self, n):
        ...
        # choose fft package
        self.irfftn = fft.irfftn
        self.rfftn = fft.rfftn
    ...
    def c2f(self, x_h):
        ...
        x_h[:,:,[0,-1]] *= sqrt(2)
        x = self.irfftn(self.extend(x_h)) * self.n32
        x_h[:,:,[0,-1]] /= sqrt(2)
        return x
    ...
    def f2c(self, x):
        ...
        x_h = self.truncate(self.rfftn(x)) / self.n32
        x_h[:,:,[0,-1]] /= sqrt(2)
        return x_h
...
class IF3D(IsoBox):
    ...
    def viscosity(self, U_h, dUdt_h):
        u_h, v_h, w_h = U_h
        dudt_h, dvdt_h, dwdt_h = dUdt_h
        dudt_h += self.nu * self.laplace * u_h
        dvdt_h += self.nu * self.laplace * v_h
        dwdt_h += self.nu * self.laplace * w_h
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        # t0 = time.time()
        u_h, v_h, w_h = ns.integrateExp(u_h, v_h, w_h, dt)
        # print time.time() - t0
        # stop
