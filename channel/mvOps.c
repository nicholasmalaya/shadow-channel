            ...
            int x0)
{
    ...
    for (i = 2; i < N; ++i) {
        ...
        for (j = MAX(0, dl - (i / 2)); j < dl; ++j) {
            ...
            for (x = x0; x < xdim; ++x) {
                ...
                Re(b[i][x]) =
                    Re(b[i][x]) - A[i][j][x] * Re(b[i - 2 * (dl - j)][x]);
                Im(b[i][x]) =
                    Im(b[i][x]) - A[i][j][x] * Im(b[i - 2 * (dl - j)][x]);
            }
        }
    ...
    for (i = N - 3; i >= 0; --i) {
        for (j = 1; j <= MIN(du, ((N - i + 1) / 2 - 1)); ++j) {
            for (x = x0; x < xdim; ++x) {
                Re(b[i][x]) =
                    Re(b[i][x]) - A[i][dl + j][x] * Re(b[i + 2 * j][x]);
                Im(b[i][x]) =
                    Im(b[i][x]) - A[i][dl + j][x] * Im(b[i + 2 * j][x]);
            }
        }
        for (x = x0; x < xdim; ++x) {
...
void bsolve0(double **A, mcomplex ** b, int dl, int du, int N)
{
    ...
    for (i = 2; i < N; ++i) {
        ...
        for (j = MAX(0, dl - (i / 2)); j < dl; ++j) {
            ...
            /* Solve Lz=b (overwritting vector b). */
            Re(b[i][0]) =
                Re(b[i][0]) - A[i][j] * Re(b[i - 2 * (dl - j)][0]);
            Im(b[i][0]) =
                Im(b[i][0]) - A[i][j] * Im(b[i - 2 * (dl - j)][0]);
        }
        ...
        for (j = 0; j < MIN(du, (N - i + 1) / 2); ++j) {
            for (k = 1; k <= MIN(i / 2, MIN(dl, du - j)); ++k) {
                A[i][j + dl] =
                    A[i][j + dl] - A[i][dl - k] * A[i - 2 * k][dl + j + k];
            }
        }
    }                           /* end for i ... */
...
void smMult(double ***A, mcomplex ** y, mcomplex ** b, int dl, int du,
            int N, int xdim, int x0)
{
    int i, j, x;
    ...
    for (i = 0; i < N; ++i) {
        for (j = MAX(0, dl - (i / 2));
             j < MIN(dl + du + 1, (N - i + 1) / 2 + dl); ++j) {
            for (x = x0; x < xdim; ++x) {
                Re(b[i][x]) =
                    Re(b[i][x]) + A[i][j][x] * Re(y[i - 2 * (dl - j)][x]);
                Im(b[i][x]) =
                    Im(b[i][x]) + A[i][j][x] * Im(y[i - 2 * (dl - j)][x]);
            }
        }
    }
}
...
void smMult0(double **A, mcomplex ** y, mcomplex * b, int dl, int du,
             int N)
{
    int i, j;
