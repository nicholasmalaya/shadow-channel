...
int LegendreSetup(void)
{
    ...
    if ((Uadd = dVector(qpts)) == NULL) {
        printf
            ("ERROR: No memory for adding term in the incremental solution.\n");
        freedMatrix(Q);
        freedMatrix(Qp);
        freedMatrix(Qpp);
        freedMatrix(R);
...
double Legendre(int n, double x)
{
    ...
    curr = 1.;
    for (j = 0; j < n; ++j) {
        y = ((double) (2 * j + 1) * x * curr -
             (double) j * prev) / (double) (j + 1);
        prev = curr;
        curr = y;
    }
    return (curr);
...
{
    int k, m, r, nd2;
    double *xe, p1, p2, p3, x1, x2, x3=0.0;
...
*/
void LdiagsMatrix(double **D, int n, int m, int start, int diags,
                  double **A, int Adim2, double **B)
{
    int i, j, k, s;
