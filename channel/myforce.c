...
void force0_2(int n, int k, int flag, double *f, double *g)
{
    ...
    for (i = 0; i < dimR; ++i) {
        ...
        if (flag == 0) {
            for (j = 0; j < qpts; ++j) {
                f[i] +=
                    Rw[i][j] * sin(2. * M_PI * Qy[j]) *
                    (cos(n * dt + e[k] * dt)
                     + re * 4. * M_PI * M_PI * sin(n * dt + dt * e[k])) +
                    Rw[i][j] * mpg;
            }
        }
    }
...
void increforce_2(int n, int k, int z, double *tmp, double *tmp2)
{
    ...
    if (z == 1 || z == Nz - 1) {
        for (i = 0; i < dimR; ++i) {
            ...
            for (j = 0; j < qpts; ++j) {
                tmp[i] +=
                    Rw[i][j] * Kz[z] * 2. * M_PI * sin(M_PI * Qy[j] / 4 +
                                                       3. * M_PI / 4) *
                    (cos(n * dt + h[k] * dt) +
                     re * (M_PI * M_PI / 16. +
                           Kz[z] * Kz[z]) * sin(n * dt + h[k] * dt));
            }
        }
    }
}
    ...
    for (i = 0; i < dimR; ++i) {
        ...
        if (z == 1) {
            for (j = 0; j < qpts; ++j) {
                f[i] +=
                    Rw[i][j] * (k11 * k11 +
                                k22 * k22) * sin(M_PI * Qy[j] * 2) *
                    (cos(n * dt + h[k] * dt) +
                     re * (4 * M_PI * M_PI + k11 * k11 +
                           k22 * k22) * sin(n * dt + h[k] * dt));
            }
        }
    }
}
...
void increforce0_3(int n, int k, int flag, double *f, double *g)
{
    ...
    for (i = 0; i < dimR; ++i) {
        ...
        for (j = 0; j < qpts; ++j) {
            f[i] +=
                k22 * Rw[i][j] * 4 * M_PI * sin(M_PI * Qy[j] / 4 +
                                                3 * M_PI / 4) * (cos(n *
                                                                     dt +
                                                                     e[k] *
                                                                     dt) +
                                                                 ...
                                                                 sin(n *
                                                                     ...
                                                                     dt));
            g[i] +=
                -k11 * Rw[i][j] * 4 * M_PI * sin(M_PI * Qy[j] / 4 +
                                                 3 * M_PI / 4) * (cos(n *
                                                                      dt +
                                                                      e[k]
                                                                      *
            ...
            for (j = 0; j < qpts; ++j) {
                tmp2[i] +=
                     ...
                     * cos(n * dt + h[k] * dt)
                     - re * (24 -
                             2 * (k11 * k11 +
                                  k22 * k22) * (12 * Qy[j] * Qy[j] - 4) +
                             (k11 * k11 + k22 * k22) * (k11 * k11 +
                                                        k22 * k22) * (1 -
                                                                      Qy[j]
                                                                      *
                                                                      Qy
                                                                      [j])
                             * (1 - Qy[j] * Qy[j])) * sin(n * dt +
                                                          h[k] * dt));
            }
        }
        for (i = 0; i < dimR; ++i) {
            ...
            for (j = 0; j < qpts; ++j) {
                tmp[i] +=
                    Rw[i][j] *
                    (((k11 * k11 + k22 * k22) * (1 - Qy[j]) +
                      k22 * (1 - Qy[j] * Qy[j]) * 4 * Qy[j]) * cos(n * dt +
                                                                   h[k] *
                                                                   dt)
                     - re * (-k22 * 24 * Qy[j] -
                             (k11 * k11 +
                              k22 * k22) * ((k11 * k11 + k22 * k22) * (1 -
                                                                       Qy
                                                                       [j])
                                            + k22 * (1 -
                                                     Qy[j] * Qy[j]) * 4 *
