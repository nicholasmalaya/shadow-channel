...
void project(int n, int k, int z, int x0, func_force_t force)
{
    ...
    if (k == 0) {               /* first step */
        ...
        smMult(M, C[z][ALPHA], TM, QSDIAG - 1, QSDIAG - 1, dimQ, Nx / 2,
               x0);
        for (i = 0; i < dimQ; ++i) {
            memcpy(&C[z][ALPHA][i][x0], &TM[i][x0],
    ...
    if (k == 0) {               /* first step */
        ...
        /* compute [Mv + (1/RE)a[k]dt*Dv]*C[z][BETA] and store the result
           in TM.  Then transfer the result back to C[z][ALPHA]. */
        smMult(M, C[z][BETA], TM, RSDIAG - 1, RSDIAG - 1, dimR, Nx / 2,
               x0);
        for (i = 0; i < dimR; ++i) {
            memcpy(&C[z][BETA][i][x0], &TM[i][x0],
                   (Nx / 2 - x0) * sizeof(mcomplex));
        }
    }
