...
void initAlphaBeta_2(int z, int x0)
{
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            ...
            Re(U[z][DXEL][i][x]) = (-Kx[x] * Im(U[z][DXEL][i][x]) +
                                    Kz[z] * Im(U[z][DZEL][i][x])) /
                K2[z][x];
            Re(U[z][DZEL][i][x]) =
                -(Kx[x] * Im(U[z][DZEL][i][x]) +
    ...
    for (x = x0; x < Nx / 2; ++x) {
        for (j = 0; j < dimR; ++j) {
            Re(HUxb[z][x]) +=
                Rpp0[j] * Im(C[z][BETA][j][x]) * Kz[z] / K2[z][x];
            Im(HUxb[z][x]) +=
                -Rpp0[j] * Re(C[z][BETA][j][x]) * Kz[z] / K2[z][x];

            Re(HUzb[z][x]) +=
                -Rpp0[j] * Im(C[z][BETA][j][x]) * Kx[x] / K2[z][x];
            Im(HUzb[z][x]) +=
                Rpp0[j] * Re(C[z][BETA][j][x]) * Kx[x] / K2[z][x];
