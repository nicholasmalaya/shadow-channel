                     ...
                     func_force_t force)
{
    ...
    extern double **Q, **Qp, **Qpp, **R, **Rp, **Qw, **Qpw, **Rw, **Qs,
        **Qps, **Qpps, **Rs, **Rps, *Rp0, **Qppw, **Rpw, *Qy;
    extern double ***M;
...
-8. / 1001., -8. / 3003., 8. / 6435. };
    ...
    if (k == 0) {               /* first step */
        ...
        smMult(M, IAC[z][ALPHA], ITM, QSDIAG - 1, QSDIAG - 1, dimQ, Nx / 2,
               x0);
        for (i = 0; i < dimQ; ++i) {
            memcpy(&IAC[z][ALPHA][i][x0], &ITM[i][x0],
        ...
        smMult(M, MIC[count][z][ALPHA], ITM, QSDIAG - 1, QSDIAG - 1, dimQ,
               Nx / 2, x0);
        for (i = 0; i < dimQ; ++i) {
            for (x = x0; x < Nx / 2; x++) {
                Re(IAC[z][ALPHA][i][x]) +=
                    Re(ITM[i][x]) * re * dt * (a[0] + b[0]);
                Im(IAC[z][ALPHA][i][x]) +=
                    Im(ITM[i][x]) * re * dt * (a[0] + b[0]);
            }
        }
        ...
        for (j = 0; j < qpts; ++j) {
            for (x = x0; x < Nx / 2; ++x) {
                Re(IFa[i][x]) += (Qpw[i][j] *
                                  ...
                                  K2[z][x] * (Qw[i][j] *
                                              Re(IU[z][HYEL][j][x]))) +
                    Qppw[i][j] * (Kz[z] * Im(IU[z][DXEL][j][x]) -
                                  Kx[x] * Im(IU[z][DZEL][j][x]));
                ...
                Im(IFa[i][x]) += (Qpw[i][j] *
                                  ...
                                  K2[z][x] * (Qw[i][j] *
                                              Im(IU[z][HYEL][j][x]))) +
                    Qppw[i][j] * (-Kz[z] * Re(IU[z][DXEL][j][x]) +
                                  Kx[x] * Re(IU[z][DZEL][j][x]));
            }
        }
    }
    if (force != NULL) {
    ...
    if (k == 0) {               /* first step */
        ...
        /* compute [Mv + (1/RE)a[k]dt*Dv]*C[z][BETA] and store the result
           in TM.  Then transfer the result back to C[z][ALPHA]. */
        smMult(M, IAC[z][BETA], ITM, RSDIAG - 1, RSDIAG - 1, dimR, Nx / 2,
               x0);
        for (i = 0; i < dimR; ++i) {
            memcpy(&IAC[z][BETA][i][x0], &ITM[i][x0],
                   (Nx / 2 - x0) * sizeof(mcomplex));
        }
        ...
        smMult(M, MIC[count][z][BETA], ITM, RSDIAG - 1, RSDIAG - 1, dimR,
               Nx / 2, x0);
        for (i = 0; i < dimR; ++i) {
            for (x = x0; x < Nx / 2; x++) {
                Re(IAC[z][BETA][i][x]) +=
        ...
        for (j = 0; j < qpts; ++j) {
            for (x = x0; x < Nx / 2; ++x) {
                Re(IFb[i][x]) += Rw[i][j] *
                    ...
                    Rpw[i][j] * (-Kz[z] * Im(IU[z][DZEL][j][x]) -
                                 Kx[x] * Im(IU[z][DXEL][j][x]));
                Im(IFb[i][x]) +=
                    Rw[i][j] * (Kz[z] * Re(IU[z][HXEL][j][x]) -
                                Kx[x] * Re(IU[z][HZEL][j][x])) +
                    Rpw[i][j] * (Kz[z] * Re(IU[z][DZEL][j][x]) +
                                 Kx[x] * Re(IU[z][DXEL][j][x]));
            }
        }
    }
    ...
    for (x = x0; x < Nx / 2; ++x) {
        for (i = 0; i < 4 && i < dimR; ++i) {
            Re(IAC[z][BETA][i][x]) +=
                g[i] / 2. * (Re(Uxbt[z][x]) - Re(Uxb[z][x])) +
                dt * re * (a[k] * Re(Uxbt[z][x]) +
                           b[k] * Re(Uxb[z][x])) * (-K2[z][x] * g[i] / 2.)
                + dt * re * (a[k] +
                             b[k]) * Re(AUxb[z][x]) * (-K2[z][x] * g[i] /
                                                       2.);
            Im(IAC[z][BETA][i][x]) +=
                g[i] / 2. * (Im(Uxbt[z][x]) - Im(Uxb[z][x])) +
                dt * re * (a[k] * Im(Uxbt[z][x]) +
                           b[k] * Im(Uxb[z][x])) * (-K2[z][x] * g[i] / 2.)
                + dt * re * (a[k] +
                             b[k]) * Im(AUxb[z][x]) * (-K2[z][x] * g[i] /
                                                       2.);
        }
    }
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            ...
            Re(IAU[z][YEL][i][x]) +=
                Re(Uzb[z][x]) * (1 - Qy[i]) * (1 - Qy[i]) * (1 +
                                                             Qy[i]) * 0.25;
            Im(IAU[z][YEL][i][x]) +=
                Im(Uzb[z][x]) * (1 - Qy[i]) * (1 - Qy[i]) * (1 +
                                                             Qy[i]) * 0.25;
        }
    }
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            for (j = 0; j < dimQ; ++j) {
                Re(IAU[z][XEL][i][x]) -=
                    Qp[i][j] * Re(IAC[z][ALPHA][j][x]);
                Im(IAU[z][XEL][i][x]) -=
                    Qp[i][j] * Im(IAC[z][ALPHA][j][x]);
            }
            Re(IAU[z][XEL][i][x]) -=
                Re(Uzb[z][x]) * (3 * Qy[i] * Qy[i] - 2 * Qy[i] - 1) * 0.25;
            Im(IAU[z][XEL][i][x]) -=
                Im(Uzb[z][x]) * (3 * Qy[i] * Qy[i] - 2 * Qy[i] - 1) * 0.25;
        }
    }
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            for (j = 0; j < dimQ; ++j) {
                Re(IAU[z][DXEL][i][x]) +=
                    Qpp[i][j] * Re(IAC[z][ALPHA][j][x]);
                Im(IAU[z][DXEL][i][x]) +=
                    Qpp[i][j] * Im(IAC[z][ALPHA][j][x]);
            }
            Re(IAU[z][DXEL][i][x]) +=
                Re(Uzb[z][x]) * (3 * Qy[i] - 1) * 0.5;
            Im(IAU[z][DXEL][i][x]) +=
                Im(Uzb[z][x]) * (3 * Qy[i] - 1) * 0.5;
        }
    }
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            for (j = 0; j < dimQ; ++j) {
                Re(IAU[z][DZEL][i][x]) +=
                    Rp[i][j] * Re(IAC[z][BETA][j][x]);
                Im(IAU[z][DZEL][i][x]) +=
                    Rp[i][j] * Im(IAC[z][BETA][j][x]);
            }
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            ...
            Re(IAU[z][XEL][i][x]) = (Kx[x] * Im(IAU[z][XEL][i][x]) +
                                     Kz[z] * Im(IAU[z][ZEL][i][x])) /
                K2[z][x];
            Re(IAU[z][ZEL][i][x]) =
                (-Kx[x] * Im(IAU[z][ZEL][i][x]) +
    ...
    for (i = 0; i < qpts; ++i) {
        for (x = x0; x < Nx / 2; ++x) {
            ...
            Re(IAU[z][DXEL][i][x]) = (-Kx[x] * Im(IAU[z][DXEL][i][x]) +
                                      Kz[z] * Im(IAU[z][DZEL][i][x])) /
                K2[z][x];
            Re(IAU[z][DZEL][i][x]) =
                -(Kx[x] * Im(IAU[z][DZEL][i][x]) +
    ...
    for (x = x0; x < Nx / 2; ++x) {
        for (j = 0; j < dimR; ++j) {
            Re(IAUxb[z][x]) +=
                (Rp0[j] * Im(IAC[z][BETA][j][x])) * Kz[z] / K2[z][x];
            Im(IAUxb[z][x]) +=
                (-Rp0[j] * Re(IAC[z][BETA][j][x])) * Kz[z] / K2[z][x];

            Re(IAUzb[z][x]) +=
                -(Rp0[j] * Im(IAC[z][BETA][j][x])) * Kx[x] / K2[z][x];
            Im(IAUzb[z][x]) +=
                (Rp0[j] * Re(IAC[z][BETA][j][x])) * Kx[x] / K2[z][x];
    ...
    for (x = x0; x < Nx / 2; ++x) {
        ...
        for (j = 0; j < dimQ; ++j) {
            sign = sign * (-1);
            Re(IAUxb[z][x]) -=
                (sign * 8. * Im(IAC[z][ALPHA][j][x])) * Kx[x] / K2[z][x];
            Im(IAUxb[z][x]) -=
                (-sign * 8. * Re(IAC[z][ALPHA][j][x])) * 2 * Kx[x] /
                K2[z][x];

            Re(IAUzb[z][x]) -=
                (sign * 8. * Im(IAC[z][ALPHA][j][x])) * Kz[z] / K2[z][x];
            Im(IAUzb[z][x]) -=
                (-sign * 8. * Re(IAC[z][ALPHA][j][x])) * 2 * Kz[z] /
                K2[z][x];
        }
    ...
    if (k != 2) {               /* not last step */
        ...
        for (i = 0; i < dimQ; ++i) {
            for (j = 0; j < qpts; ++j) {
                for (x = x0; x < Nx / 2; ++x) {
                    Re(IFa[i][x]) += (Qpw[i][j] *
                                      (-Kx[x] * Im(LIU[z][HXEL][j][x]) -
                                       Kz[z] * Im(LIU[z][HZEL][j][x])) -
                                      K2[z][x] * (Qw[i][j] *
                                                  Re(LIU[z][HYEL][j][x])))
                        + Qppw[i][j] * (Kz[z] * Im(LIU[z][DXEL][j][x]) -
                                        Kx[x] * Im(LIU[z][DZEL][j][x]));

                    Im(IFa[i][x]) += (Qpw[i][j] *
                                      (Kx[x] * Re(LIU[z][HXEL][j][x]) +
                                       Kz[z] * Re(LIU[z][HZEL][j][x])) -
                                      K2[z][x] * (Qw[i][j] *
                                                  Im(LIU[z][HYEL][j][x])))
                        + Qppw[i][j] * (-Kz[z] * Re(LIU[z][DXEL][j][x]) +
                                        Kx[x] * Re(LIU[z][DZEL][j][x]));
                }
            }
        }
        if (force != NULL) {
        ...
        smMult(M, MIC[count - 1][z][ALPHA], ITM, QSDIAG - 1, QSDIAG - 1,
               dimQ, Nx / 2, x0);

        for (i = 0; i < dimQ; ++i) {
            for (x = x0; x < Nx / 2; x++) {
                Re(IAC[z][ALPHA][i][x]) +=
                    Re(ITM[i][x]) * re * dt * (a[k + 1] + b[k + 1]);
                Im(IAC[z][ALPHA][i][x]) +=
                    Im(ITM[i][x]) * re * dt * (a[k + 1] + b[k + 1]);
            }
        }
        ...
        for (i = 0; i < dimR; ++i) {
            for (j = 0; j < qpts; ++j) {
                for (x = x0; x < Nx / 2; ++x) {
                    Re(IFb[i][x]) += Rw[i][j] *
                        (-Kz[z] * Im(LIU[z][HXEL][j][x]) +
                         Kx[x] * Im(LIU[z][HZEL][j][x])) +
                        Rpw[i][j] * (-Kz[z] * Im(LIU[z][DZEL][j][x]) -
                                     Kx[x] * Im(LIU[z][DXEL][j][x]));
                    Im(IFb[i][x]) +=
                        Rw[i][j] * (Kz[z] * Re(LIU[z][HXEL][j][x]) -
                                    Kx[x] * Re(LIU[z][HZEL][j][x])) +
                        Rpw[i][j] * (Kz[z] * Re(LIU[z][DZEL][j][x]) +
                                     Kx[x] * Re(LIU[z][DXEL][j][x]));
                }
            }
        }
        ...
        smMult(M, MIC[count - 1][z][BETA], ITM, RSDIAG - 1, RSDIAG - 1,
               dimR, Nx / 2, x0);
        for (i = 0; i < dimR; ++i) {
            for (x = x0; x < Nx / 2; x++) {
                Re(IAC[z][BETA][i][x]) +=
