...
int pass2(int dctr, int n)
{
    ...
    for (y = 0; y < qpts; ++y) {
        ...
        for (i = 0; i < 3; ++i) {
            for (z = 0; z < Nz / 2; ++z) {
                memcpy(CT[i + MAXT][z], AU[z][i][y],
                       (Nx / 2) * sizeof(fftw_complex));
            }
        }
        for (i = 0; i < 3; ++i) {
            for (z = Nz / 2 + 1; z < Nz; ++z) {
                memcpy(CT[i + MAXT][z + Nz / 2], AU[z][i][y],
                       (Nx / 2) * sizeof(fftw_complex));
            }
        }
        ...
        for (i = 0; i < 3; ++i) {
            for (z = 0; z < Nz / 2; ++z) {
                memcpy(ICT[i + MAXT][z], IAU[z][i][y],
                       (Nx / 2) * sizeof(fftw_complex));
            }
        }
        for (i = 0; i < 3; ++i) {
            for (z = Nz / 2 + 1; z < Nz; ++z) {
                memcpy(ICT[i + MAXT][z + Nz / 2], IAU[z][i][y],
                       (Nx / 2) * sizeof(fftw_complex));
            }
        }
        ...
        for (z = 0; z < (3 * Nz / 2); ++z) {
            for (x = 0; x < 3 * Nx / 2; ++x) {
                ...
                IRT[SXEL * idx + (z * (3 * Nx / 2 + 2) + x)] =
                    RT[YEL * idx +
                       (z * (3 * Nx / 2 + 2) + x)] * IRT[AZEL * idx +
                                                         (z *
                                                          (3 * Nx / 2 +
                                                           2) + x)] -
                    RT[ZEL * idx +
                       (z * (3 * Nx / 2 + 2) + x)] * IRT[AYEL * idx +
                                                         (z *
                                                          (3 * Nx / 2 +
                                                           2) + x)] +
                    IRT[YEL * idx +
                        (z * (3 * Nx / 2 + 2) + x)] * RT[AZEL * idx +
                                                         (z *
                                                          (3 * Nx / 2 +
                                                           2) + x)] -
                    IRT[ZEL * idx +
                        (z * (3 * Nx / 2 + 2) + x)] * RT[AYEL * idx +
                                                         (z *
                                                          (3 * Nx / 2 +
                                                           2) + x)];
        ...
        for (z = 0; z < Nz / 2; ++z) {
            memcpy(U[z][DXEL][y], CT[SXEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            memcpy(U[z][DZEL][y], CT[SZEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            memcpy(IU[z][DXEL][y], ICT[SXEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            memcpy(IU[z][DZEL][y], ICT[SZEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            for (x = 0; x < Nx / 2; ++x) {
                Re(U[z][HXEL][y][x]) =
                    -Re(CT[TXEL][z][x]) - Kz[z] * Im(CT[SYEL][z][x]);
                Im(U[z][HXEL][y][x]) =
                    -Im(CT[TXEL][z][x]) + Kz[z] * Re(CT[SYEL][z][x]);
                Re(U[z][HYEL][y][x]) =
                    -Re(CT[TYEL][z][x]) + Kz[z] * Im(CT[SXEL][z][x]) -
                    Kx[x] * Im(CT[SZEL][z][x]);
                Im(U[z][HYEL][y][x]) =
                    -Im(CT[TYEL][z][x]) - Kz[z] * Re(CT[SXEL][z][x]) +
                    Kx[x] * Re(CT[SZEL][z][x]);
                Re(U[z][HZEL][y][x]) =
                    -Re(CT[TZEL][z][x]) + Kx[x] * Im(CT[SYEL][z][x]);
                Im(U[z][HZEL][y][x]) =
                    -Im(CT[TZEL][z][x]) - Kx[x] * Re(CT[SYEL][z][x]);

                Re(IU[z][HXEL][y][x]) =
                    -Re(ICT[TXEL][z][x]) - Kz[z] * Im(ICT[SYEL][z][x]);
                Im(IU[z][HXEL][y][x]) =
                    -Im(ICT[TXEL][z][x]) + Kz[z] * Re(ICT[SYEL][z][x]);
                Re(IU[z][HYEL][y][x]) =
                    -Re(ICT[TYEL][z][x]) + Kz[z] * Im(ICT[SXEL][z][x]) -
                    Kx[x] * Im(ICT[SZEL][z][x]);
                Im(IU[z][HYEL][y][x]) =
                    -Im(ICT[TYEL][z][x]) - Kz[z] * Re(ICT[SXEL][z][x]) +
                    Kx[x] * Re(ICT[SZEL][z][x]);
                Re(IU[z][HZEL][y][x]) =
                    -Re(ICT[TZEL][z][x]) + Kx[x] * Im(ICT[SYEL][z][x]);
                Im(IU[z][HZEL][y][x]) =
                    -Im(ICT[TZEL][z][x]) - Kx[x] * Re(ICT[SYEL][z][x]);
            }
        }

        for (z = Nz + 1; z < 3 * Nz / 2; ++z) {
            memcpy(U[z - Nz / 2][DXEL][y], CT[SXEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            memcpy(U[z - Nz / 2][DZEL][y], CT[SZEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            memcpy(IU[z - Nz / 2][DXEL][y], ICT[SXEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            memcpy(IU[z - Nz / 2][DZEL][y], ICT[SZEL][z],
                   Nx / 2 * sizeof(fftw_complex));
            for (x = 0; x < Nx / 2; ++x) {
                Re(U[z - Nz / 2][HXEL][y][x]) =
                    -Re(CT[TXEL][z][x]) - Kz[z -
                                             Nz / 2] * Im(CT[SYEL][z][x]);
                Im(U[z - Nz / 2][HXEL][y][x]) =
                    -Im(CT[TXEL][z][x]) + Kz[z -
                                             Nz / 2] * Re(CT[SYEL][z][x]);
                Re(U[z - Nz / 2][HYEL][y][x]) =
                    -Re(CT[TYEL][z][x]) + Kz[z -
                                             Nz / 2] * Im(CT[SXEL][z][x]) -
                    Kx[x] * Im(CT[SZEL][z][x]);
                Im(U[z - Nz / 2][HYEL][y][x]) =
                    -Im(CT[TYEL][z][x]) - Kz[z -
                                             Nz / 2] * Re(CT[SXEL][z][x]) +
                    Kx[x] * Re(CT[SZEL][z][x]);
                Re(U[z - Nz / 2][HZEL][y][x]) =
                    -Re(CT[TZEL][z][x]) + Kx[x] * Im(CT[SYEL][z][x]);
                Im(U[z - Nz / 2][HZEL][y][x]) =
                    -Im(CT[TZEL][z][x]) - Kx[x] * Re(CT[SYEL][z][x]);

                Re(IU[z - Nz / 2][HXEL][y][x]) =
                    -Re(ICT[TXEL][z][x]) - Kz[z -
                                              Nz / 2] *
                    Im(ICT[SYEL][z][x]);
                Im(IU[z - Nz / 2][HXEL][y][x]) =
                    -Im(ICT[TXEL][z][x]) + Kz[z -
                                              Nz / 2] *
                    Re(ICT[SYEL][z][x]);
                Re(IU[z - Nz / 2][HYEL][y][x]) =
                    -Re(ICT[TYEL][z][x]) + Kz[z -
                                              Nz / 2] *
                    Im(ICT[SXEL][z][x]) - Kx[x] * Im(ICT[SZEL][z][x]);
                Im(IU[z - Nz / 2][HYEL][y][x]) =
                    -Im(ICT[TYEL][z][x]) - Kz[z -
                                              Nz / 2] *
                    Re(ICT[SXEL][z][x]) + Kx[x] * Re(ICT[SZEL][z][x]);
                Re(IU[z - Nz / 2][HZEL][y][x]) =
                    -Re(ICT[TZEL][z][x]) + Kx[x] * Im(ICT[SYEL][z][x]);
                Im(IU[z - Nz / 2][HZEL][y][x]) =
                    -Im(ICT[TZEL][z][x]) - Kx[x] * Re(ICT[SYEL][z][x]);
            }
        }
    }                           /* end for y... */
