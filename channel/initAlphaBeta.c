...
void Beta(int z, int x0, int xdim)
{
    ...
    for (x = x0; x < xdim; ++x) {
        for (j = 0; j < dimR; ++j) {
            Re(Uxb[z][x]) +=
                Rp0[j] * Im(C[z][BETA][j][x]) * Kz[z] / K2[z][x];
            Im(Uxb[z][x]) +=
                -Rp0[j] * Re(C[z][BETA][j][x]) * Kz[z] / K2[z][x];

            Re(Uzb[z][x]) +=
                -Rp0[j] * Im(C[z][BETA][j][x]) * Kx[x] / K2[z][x];
            Im(Uzb[z][x]) +=
                Rp0[j] * Re(C[z][BETA][j][x]) * Kx[x] / K2[z][x];
        }
    }
             ...
             int x0)
{
    ...
    for (i = 2; i < N; ++i) {
        ...
        for (j = MAX(0, dl - (i / 2)); j < dl; ++j) {
            ...
            for (x = x0; x < xdim; ++x) {
                Re(b[i][x]) =
                    Re(b[i][x]) - A[i][j] * Re(b[i - 2 * (dl - j)][x]);
                Im(b[i][x]) =
                    Im(b[i][x]) - A[i][j] * Im(b[i - 2 * (dl - j)][x]);
            }
        }
    ...
    for (i = N - 3; i >= 0; --i) {
        for (j = 1; j <= MIN(du, ((N - i + 1) / 2 - 1)); ++j) {
            for (x = x0; x < xdim; ++x) {
                Re(b[i][x]) =
                    Re(b[i][x]) - A[i][dl + j] * Re(b[i + 2 * j][x]);
                Im(b[i][x]) =
                    Im(b[i][x]) - A[i][dl + j] * Im(b[i + 2 * j][x]);
            }
        }
        for (x = x0; x < xdim; ++x) {
