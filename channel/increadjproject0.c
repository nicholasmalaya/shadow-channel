...
void increadjproject0(int k, int n, int count, func_force_t force)
{
    ...
    if (k == 0) {               /* first step */
        ...
        smMult0(Rps, MIC[count][0][ALPHA], Itm, RSDIAG - 1, RSDIAG - 1,
                dimR);
        for (i = 0; i < dimR; ++i) {
            Re(IAC[0][ALPHA][i][0]) -=
                Re(Itm[i]) * dt * re * (a[0] + b[0]);
            Im(IAC[0][ALPHA][i][0]) -=
                Im(Itm[i]) * dt * re * (a[0] + b[0]);
        }
        ...
        smMult0(Rps, MIC[count][0][BETA], Itm, RSDIAG - 1, RSDIAG - 1,
                dimR);
        for (i = 0; i < dimR; ++i) {
    ...
    for (i = 0; i < dimR; ++i) {
        for (j = 0; j < qpts; ++j) {
            Re(Ifa[i]) +=
                Rw[i][j] * (Re(IU[0][HXEL][j][0])) +
                Rpw[i][j] * Re(IU[0][DZEL][j][0]);
        }
        if (force != NULL) {
            Re(Ifa[i]) += Re(tmp[i]);
            Im(Ifa[i]) += Im(tmp[i]);
    ...
    for (i = 0; i < dimR; ++i) {
        for (j = 0; j < qpts; ++j) {
            Re(Ifb[i]) +=
                Rw[i][j] * Re(IU[0][HZEL][j][0]) -
                Rpw[i][j] * Re(IU[0][DXEL][j][0]);
        }
        if (force != NULL) {
            Re(Ifb[i]) += Re(tmp2[i]);
            Im(Ifb[i]) += Im(tmp2[i]);
    ...
    if (k != 2) {               /* not last step */
        ...
        for (i = 0; i < dimR; ++i) {
            for (j = 0; j < qpts; ++j) {
                Re(Ifa[i]) +=
                    Rw[i][j] * (Re(LIU[0][HXEL][j][0])) +
                    Rpw[i][j] * Re(LIU[0][DZEL][j][0]);
            }
            if (force != NULL) {
                Re(Ifa[i]) += Re(tmp[i]);
                Im(Ifa[i]) += Im(tmp[i]);
        ...
        smMult0(Rps, MIC[count - 1][0][ALPHA], Itm, RSDIAG - 1, RSDIAG - 1,
                dimR);
        for (i = 0; i < dimR; ++i) {
            Re(IAC[0][ALPHA][i][0]) -=
                Re(Itm[i]) * dt * re * (a[k + 1] + b[k + 1]);
            Im(IAC[0][ALPHA][i][0]) -=
                Im(Itm[i]) * dt * re * (a[k + 1] + b[k + 1]);
        }
        ...
        for (i = 0; i < dimR; ++i) {
            for (j = 0; j < qpts; ++j) {
                Re(Ifb[i]) +=
                    Rw[i][j] * Re(LIU[0][HZEL][j][0]) -
                    Rpw[i][j] * Re(LIU[0][DXEL][j][0]);
            }
            if (force != NULL) {
                Re(Ifb[i]) += Re(tmp2[i]);
                Im(Ifb[i]) += Im(tmp2[i]);
        ...
        smMult0(Rps, MIC[count - 1][0][BETA], Itm, RSDIAG - 1, RSDIAG - 1,
                dimR);
        for (i = 0; i < dimR; ++i) {
            Re(IAC[0][BETA][i][0]) -=
                Re(Itm[i]) * dt * re * (a[k + 1] + b[k + 1]);
